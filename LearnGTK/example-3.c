...
// Create a new surface of the appropriate size to store our scirbbles
...
// Redraw the screen from the surface. Note that the draw callback receives a ready-to-be-used cairo_t
// that is already clipped to only draw the exposed
...
// Draw a recangle on the surface at the given position
...
static void activate(GtkApplication* app, gpointer user_date)
{
    ...
    gtk_widget_set_size_request(drawing_area, 100, 100); // set a minimum size
